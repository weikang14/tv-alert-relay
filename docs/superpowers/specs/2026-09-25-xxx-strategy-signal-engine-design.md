...
时候,图表周期从 1 分钟改为 15 分钟——见第一节表格及全文相应处的更新。同日再
后来,连上 TradingView Desktop 用真实图表数据核对后发现桶边界的对齐基准原来
理解也是错的——不是 UTC 零点,而是纽约时间 17:00、随夏令时切换,已更正,详见
第五节第 7 点;另发现 `poll_once()` 里 TP/SL 判断跟入场/反向处理没有按时间顺序
交替进行,导致一次轮询里出现多笔入场时前面的信号永远拿不到自己的 TP/SL 判断
机会,已改成按时间顺序交替处理)
...
6. **拉取根数(`outputsize`)必须覆盖完整一个桶,不能只看轮询间隔(2026-09-25
   ...
   本该一直存在的换算步骤。)
7. **桶边界的对齐基准是纽约时间 17:00,不是 UTC 零点(2026-09-25 补,连上
   TradingView Desktop 用真实图表数据核对后发现)**:`_bucket_start()` 原来的
   实现是按 Unix epoch 对齐(`epoch_minutes // BUCKET_MINUTES`),对 120 分钟
   桶来说效果等于对齐到 UTC 整点(00:00、02:00、04:00...)。用 TradingView
   Desktop 实际拉取 `OANDA:XAUUSD` 的 120 分钟原生 K 线核对,发现真实边界落在
   21:00 UTC(夏令时期间)或 22:00 UTC(冬令时期间),而且切换时间点精确对应
   美国夏令时的转换日期(2025-11-02、2026-03-08)——也就是外汇/黄金交易日的
   惯例起点"纽约时间下午 5 点",不是 UTC 零点,而且这个 UTC 偏移量本身每年
   随夏令时切换两次,不是固定值。已改正:`_bucket_start()` 现在先把时间转到
   `America/New_York` 时区判断属于哪个交易日(以当地 17:00 为分界),再把
   那个交易日的 17:00 换算回 UTC 作为该交易日第一个桶的起点,后续桶依次加
   `BUCKET_MINUTES`——用 Python 标准库 `zoneinfo` 处理夏令时换算,`requirements.txt`
   加了 `tzdata` 包保证 Render 部署环境(Linux)一定能查到时区数据库。这个偏差
   之前从来没被验证过(此前没有可用的 TradingView Desktop 连接),用批量历史
